# Postmortem: From Snapshot as Fast State to Snapshot Trust Contract

[← Back to Postmortems Index](README.md)

**Recorded on:** 2026-06-05
Updated for Stage 3.5D: 2026-06-13

## 1. Context

While planning the later stages of the Streaming System + Compass project, I started thinking about snapshot support.

The original motivation seemed straightforward:

```text
event log grows over time
→ full replay becomes expensive
→ snapshot can reduce replay / rehydrate cost
```

At first, snapshot looked like a simple performance optimization.

However, the more I examined it, the more I realized that snapshot introduces a deeper correctness problem:

```text
If snapshot state is mutable, stale, incompatible, corrupted, or generated by non-deterministic logic,
the runtime may derive the wrong current state.
```

This matters because Compass depends on trusted reconstruction of accepted history.

If the state used by Compass is already polluted, later validation may become misleading.

---

## 2. Problem

The main problem was a tension between performance and trust.

On one side, snapshot exists to avoid full replay:

```text
load snapshot
→ replay tail events
→ derive current state faster
```

On the other side, if a snapshot can be modified after creation, generated by outdated logic, hashed non-deterministically, or disconnected from accepted history, it may become dangerous.

For example, accepted event history may imply:

```text
status = CREATED
```

but a snapshot row may say:

```text
status = PAID
```

If runtime logic blindly trusts that snapshot, then the system may derive a false state.

This could cause several downstream problems:

- valid commands may be wrongly rejected
- invalid commands may appear valid
- Compass validation context may be polluted
- read-side state may no longer reflect accepted history
- future action-safety decisions may be based on false derived state

The naive answer would be:

```text
Always full replay and compare snapshot with event log.
```

But that defeats the purpose of snapshot.

If every normal runtime path must perform full replay to verify the snapshot, then snapshot no longer provides meaningful replay efficiency.

---

## 3. Root Cause

The root cause was treating snapshot as a simple cached state without first defining its trust boundary.

I was mixing two different questions:

```text
How can snapshot make replay faster?
```

and:

```text
Why is this snapshot safe enough to use?
```

A snapshot is not the source of truth.

It is derived state.

But if runtime logic uses it as the starting point for reconstruction, then it still needs a trust contract.

The missing concept was:

```text
Snapshot should not be blindly trusted,
but it should not require full replay on every normal path either.
```

A second missing distinction was:

```text
snapshot consumption
≠
snapshot production
```

A validator should decide whether an existing snapshot can be used.

A builder should create snapshot payload from trusted reconstruction output.

A generation policy should decide when a new snapshot should be produced.

These responsibilities should not be collapsed into the store or validator.

---

## 4. What I Realized

The correct model is not:

```text
snapshot is always trusted
```

or:

```text
snapshot must always be fully verified by replay
```

The better model is:

```text
snapshot = fast path
event log replay = authority path
```

Normal runtime can use snapshot when it passes lightweight trust checks.

Full replay should be reserved for:

- audit
- debugging
- suspicious snapshot state
- hash / metadata failure
- reducer or aggregate logic version changes
- high-risk action verification
- rebuild / recovery
- later Layer 2 validation

This means snapshot trust is not binary.

A snapshot may have different trust states:

- invalid / untrusted
- fast-path eligible
- high-confidence
- recently audited

The key realization is:

```text
Fast checks do not prove full semantic equivalence.
They decide whether the snapshot is qualified to be used on the fast path.
```

Full replay remains the final authority, but it does not need to run every time.

---

## 5. Resolution

The resolution is to treat snapshot as a derived state-compression artifact with an explicit Snapshot Trust Contract.

A future snapshot should include more than only state payload.

It should carry lineage and integrity evidence such as:

```text
snapshot_id
snapshot_kind
aggregate_id / order_id
source_event_id
source_event_sequence
source_global_position
snapshot_schema_version
logic_version
payload_hash
created_at
created_by
metadata_json
```

For projection snapshots, `logic_version` may represent reducer version.

For write-side aggregate snapshots, `logic_version` may represent aggregate rehydration logic version.

The runtime can then perform fast trust checks before using the snapshot.

### 5.1 Lineage Check

The snapshot should point back to accepted history.

Check:

```text
snapshot.order_id == target order_id
source_event_id exists
source event belongs to the same stream
source_event_sequence matches the source event sequence
source_global_position matches the source event global_position
snapshot source position is not ahead of latest accepted event
```

This prevents ungrounded snapshots.

### 5.2 Version Compatibility Check

Snapshot state version should not be assumed to equal source event sequence in all future domains.

Some future accepted events may not advance business state version.

Therefore, database constraints should only enforce physical plausibility:

```text
state_version >= 0
state_version <= source_event_sequence
```

Stronger version semantics should be checked by the relevant runtime trust validator.

### 5.3 Tail Continuity Check

If snapshot is at version or sequence boundary `N`, and latest accepted event is sequence `M`, the tail replay must contain:

```text
N + 1
through
M
```

if the event stream contract requires contiguous sequence for tail replay.

If any required version is missing, snapshot-assisted replay should not be trusted.

For projection snapshots, global event-log continuity may also matter because the projection worker consumes accepted history by global position.

### 5.4 Canonical Payload Hash Check

The snapshot payload should have a hash over a canonical payload.

The hash must not depend on incidental Python dictionary insertion order, whitespace, or arbitrary object serialization.

The payload should be converted into canonical primitive values first.

Examples:

```text
Decimal("100.00") → "100.00"
UUID → canonical lowercase string
Enum → stable value
dict keys → sorted
JSON separators → no insignificant whitespace
```

Then hash a deterministic JSON representation.

This is not full semantic proof, but it protects against accidental corruption, simple tampering, and non-deterministic serialization bugs.

### 5.5 Schema and Logic Version Check

A snapshot should record which schema version and logic version produced it.

If a reducer version or aggregate rehydration logic version is later known to be wrong, snapshots generated by that version should be ignored and rebuilt.

This handles the case where the snapshot was not tampered with, but was produced by flawed logic.

### 5.6 Idempotent Snapshot Write

Snapshot write should tolerate benign races.

If two workers attempt to persist the same snapshot boundary with the same hash, the second write should be treated as idempotent success.

If two workers attempt to persist the same snapshot boundary with different hashes, the system should raise a collision error.

```text
same boundary + same payload_hash = idempotent success
same boundary + different payload_hash = SnapshotWriteCollisionError
```

### 5.7 Fallback to Full Replay

If any trust check fails:

```text
ignore snapshot
→ full replay accepted history
→ optionally rebuild snapshot through an explicit rebuild path
→ optionally emit future SemanticOutcome
```

The snapshot must be discardable.

Accepted event history remains the authority.

---

## 6. Key Takeaway

Snapshot is not just a performance feature.

It creates a trust boundary.

The correct question is not only:

```text
How can snapshot reduce replay cost?
```

but also:

```text
Why is this snapshot safe enough to use on the fast path?
```

The final principle is:

```text
Snapshot is the fast path.
Event log replay is the authority path.
```

A snapshot may accelerate reconstruction, but it must remain:

- traceable to accepted history
- checkable through metadata and integrity evidence
- rejectable when invalid
- discardable when suspicious
- rebuildable from the event log
- written idempotently under benign races
- protected against non-deterministic hash behavior

---

## 7. Reusable Lesson

For future architecture work, derived state should not be treated as harmless merely because it can be rebuilt.

Derived state can still cause runtime damage if it is trusted at the wrong moment.

A safer reasoning sequence is:

1. Identify the source of truth.
2. Identify which states are derived.
3. Identify where derived state is used by runtime decisions.
4. Define fast-path trust checks.
5. Define canonical integrity evidence.
6. Define authority-path verification.
7. Define fallback / rebuild behavior.
8. Define idempotent write collision policy.
9. Only then optimize for replay efficiency.

This applies not only to projection snapshots and aggregate snapshots, but also to:

- read-side projections
- materialized views
- checkpointed state
- agent-visible derived state
- future isolated read-side databases

---

## 8. Follow-up Action

This issue should be carried into Stage 3.5D roadmap planning.

### Stage 3.5D

Introduce snapshot and replay-efficiency work.

The first implementation pass should focus on projection / read-side snapshot trust because Stage 3.5C has completed the durable read-side baseline.

A later Stage 3.5D extension may apply the same contract to write-side aggregate snapshot-assisted rehydration.

Potential tasks:

```text
general snapshot trust contract
projection snapshot schema
snapshot metadata / lineage
canonical snapshot payload hashing
snapshot write collision policy
snapshot generation policy
snapshot-assisted projection replay
tail replay continuity check
snapshot_schema_version
reducer_version
payload_hash
fallback to full replay
aggregate snapshot trust extension
aggregate snapshot schema / store
snapshot-assisted write-side rehydration
write-side admission-path safety tests
```

### Stage 4

Convert snapshot / projection trust failures into structured outcomes.

Potential future error types:

```text
SNAPSHOT_METADATA_INVALID
SNAPSHOT_HASH_MISMATCH
SNAPSHOT_SCHEMA_UNSUPPORTED
SNAPSHOT_LOGIC_VERSION_UNTRUSTED
SNAPSHOT_TAIL_DISCONTINUITY
SNAPSHOT_REPLAY_MISMATCH
SNAPSHOT_WRITE_COLLISION
PROJECTION_DRIFT
```

### Stage 5+

Consider stronger governance hardening only after the core flow works.

Possible future directions:

```text
sealed milestone snapshot
HMAC / digital signature
isolated derived-state runtime
oblivious agent runtime
controlled agent read boundary
```

These are not current implementation requirements.

They are future hardening directions.
